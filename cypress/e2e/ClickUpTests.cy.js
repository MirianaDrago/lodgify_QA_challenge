...
describe('Click Up Scenarios', () => {
    describe('Scenario 1: Validate Create Space functionality', () => {
        ...
        // delete the workspace created as clean up
        after(() => {
            cy.get('@createdSpaceId').then(createdSpaceId => {
                if (createdSpaceId) {
                    cy.deleteSpace(createdSpaceId);  // delete the space if it was created
                } else {
                    cy.log('No space was created or found, skipping deletion');
                }
            });
        });
    });
